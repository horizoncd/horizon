...
func (c *controller) GetAPIDashboard(_ context.Context, internal string) (string, error) {
	...
	if ok {
		...
		// &var-range=%s&var-api_availability=%s&var-api_read_rt=%s&var-api_write_rt=%s
...
func (c *controller) GetPipelineDashboard(_ context.Context, internal string, env string) (string, error) {
	availability, ok := c.GrafanaSLO.Availability[internal]
	if ok {
		// http://grafana.mockserver.org/d/g40XAtbnk/horizon-pipeline-slo?orgId=1&kiosk&theme=light
		// &var-env=%s&var-git_availability=%s&var-git_rt=%s&var-image_availability=%s&var-image_rt=%s
		// &var-range=%s&var-deploy_availability=0.99&var-deploy_rt=60
