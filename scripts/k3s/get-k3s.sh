...
PROXY_GITHUB_URL=https://ghproxy.com/${GITHUB_URL}
STORAGE_URL=https://k3s-ci-builds.s3.amazonaws.com
DOWNLOADER=

echo "============>"  ${PROXY_GITHUB_URL}
...
download_hash() {
    if [ -n "${INSTALL_K3S_COMMIT}" ]; then
        ...
        HASH_URL=${PROXY_GITHUB_URL}/download/${VERSION_K3S}/sha256sum-${ARCH}.txt
    fi
...
download_binary() {
    if [ -n "${INSTALL_K3S_COMMIT}" ]; then
        ...
        BIN_URL=${PROXY_GITHUB_URL}/download/${VERSION_K3S}/k3s${SUFFIX}
    fi
