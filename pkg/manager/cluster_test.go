...
import (
	...
	userdao "github.com/horizoncd/horizon/pkg/dao"
...
func TestCluster(t *testing.T) {
	var (
		...
		ctx       context.Context
		mgr       = NewClusterManager(db)
		memberMgr = NewMemberManager(db)
	...
	if err := db.AutoMigrate(&models.Cluster{}, &models.Tag{}, &models.User{},
		&models.EnvironmentRegion{}, &models.Region{}, &models.Member{},
		&models.Pipelinerun{}); err != nil {
		panic(err)
	}
	userDAO := userdao.NewUserDAO(db)
	_, err := userDAO.Create(ctx, tony)
	if err != nil {
		panic(err)
	}
	ctx = context.TODO()
	...
	// test
	user2, err := userDAO.Create(ctx, &models.User{
		Name:  "leo",
		Email: "leo@corp.com",
	})
	assert.Nil(t, err)
	...
	cluster, err = mgr.Create(ctx, cluster, []*models.Tag{
		{
			Key:   "k1",
			Value: "v1",
		},
	...
	clusterMembers, err := memberMgr.ListDirectMember(ctx, models.TypeApplicationCluster, cluster.ID)
	assert.Nil(t, err)
	...
	_, err = mgr.Create(ctx, cluster2, []*models.Tag{
		{
			Key:   "k1",
			Value: "v3",
		},
	...
	query = q.New(q.KeyWords{
		common.ParamApplicationID: applicationID,
		common.ClusterQueryTagSelector: []models.TagSelector{
			{
				Key:      "k1",
				Operator: models.In,
	...
	_, err = mgr.Create(ctx, cluster, []*models.Tag{
		{
			Key:   "k1",
			Value: "v3",
		},
