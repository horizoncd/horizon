...
func createOAuthCtx() (context.Context, *gorm.DB, tokenstorage.Storage,
	oauthdao.OAuthDAO, OAuthManager, TokenManager, userauth.User) {
	var (
		db           *gorm.DB
	...
	oauthManager = NewOAuthManager(oauthAppDAO, tokenStorage, generator.NewOauthAccessGenerator(),
		authorizeCodeExpireIn, accessTokenExpireIn)
	tokenManager = NewTokenManager(db)
	return ctx, db, tokenStorage, oauthAppDAO, oauthManager, tokenManager, aUser
}
...
func TestOauthAppBasic(t *testing.T) {
	ctx, _, _, _, oauthManager, _, _ := createOAuthCtx()
...
func TestClientSecretBasic(t *testing.T) {
	ctx, _, _, _, oauthManager, _, aUser := createOAuthCtx()
	// create app
	createReq := &CreateOAuthAppReq{
		Name:        "OauthTest",
		RedirectURI: "https://example.com/oauth/redirect",
...
func TestOauthAuthorizeAndAccessBasic(t *testing.T) {
	ctx, _, _, _, oauthManager, tokenManager, _ := createOAuthCtx()
	// create app
	createReq := &CreateOAuthAppReq{
		Name:        "OauthTest",
		RedirectURI: "https://example.com/oauth/redirect",
