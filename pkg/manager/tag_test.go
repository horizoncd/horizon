...
func createTagCtx() (context.Context, TagManager, *gorm.DB) {
	var (
		db, _ = orm.NewSqliteDB("")
		ctx   context.Context
		mgr   = NewTagManager(db)
	)
...
func Test_ValidateTagsUpsert(t *testing.T) {
	tags := make([]*models.Tag, 0)
	tags = append(tags, &models.Tag{
		Key:   strings.Repeat("a", 63),
		Value: "a",
	})
	err := ValidateTagsUpsert(tags)
	assert.Nil(t, err)
