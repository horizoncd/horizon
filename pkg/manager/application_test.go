...
import (
	...
	groupdao "github.com/horizoncd/horizon/pkg/dao"
...
func createApplicationCtx() (context.Context, ApplicationManager, MemberManager, *gorm.DB) {
	var (
		db, _     = orm.NewSqliteDB("")
		ctx       context.Context
		mgr       = NewApplicationManager(db)
	...
	userDAO := groupdao.NewUserDAO(db)
	_, err := userDAO.Create(ctx, &currentUser)
	if err != nil {
		panic(err)
	}
	return ctx, mgr, memberMgr, db
}

func TestApplication(t *testing.T) {
	ctx, mgr, memberMgr, db := createApplicationCtx()
	...
	groupDAO := groupdao.NewGroupDAO(db)
	group, err := groupDAO.Create(ctx, &models.Group{
		Model:    global.Model{},
		Name:     "group1",
		Path:     "/group1",
		ParentID: 0,
	...
	group1, err := groupDAO.Create(ctx, &models.Group{
		Model:    global.Model{},
		Name:     "group2",
		Path:     "/group2",
		ParentID: 0,
	...
	clusterMembers, err := memberMgr.ListDirectMember(ctx, models.TypeApplication, application.ID)
	assert.Nil(t, err)
...
func TestListApplication(t *testing.T) {
	ctx, mgr, _, db := createApplicationCtx()
	currentUser, err := common.UserFromContext(ctx)
	assert.Nil(t, err)
	userDAO := groupdao.NewUserDAO(db)
	user2, err := userDAO.Create(ctx, &models.User{
