...
func createTokenCtx() (context.Context, *gorm.DB, TokenManager, generator.AccessTokenCodeGenerator, userauth.User) {
	var (
		db                       *gorm.DB
	...
	return ctx, db, tokenManager, userAccessTokenGenerator, aUser
}

func TestTokenBasic(t *testing.T) {
	ctx, _, tokenManager, userAccessTokenGenerator, aUser := createTokenCtx()
	// create
