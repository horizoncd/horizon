...
func createUserCtx() (context.Context, manager.UserManager, *managerparam.Manager, *gorm.DB) {
	var (
	...
	if err := db.AutoMigrate(&idpmodels.User{},
		&idpmodels.IdentityProvider{}, &idpmodels.UserLink{}); err != nil {
		panic(err)
	}
	ctx = context.TODO()
	return ctx, mgr, mgrs, db
}

func TestUser(t *testing.T) {
	ctx, mgr, mgrs, _ := createUserCtx()
	var (
		name  = "Tony"
		email = "tony@163.com"
	)
	...
	u, err := mgr.Create(ctx, &idpmodels.User{
		Name:  name,
		Email: email,
	})
	if err != nil {
...
func TestSearchUser(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		_, err = mgr.Create(ctx, &idpmodels.User{
