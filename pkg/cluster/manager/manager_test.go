...
func Test(t *testing.T) {
	...
	count, clustersWithEnvAndRegion, err := Mgr.ListByApplicationAndEnvs(ctx, applicationID,
		[]string{cluster.EnvironmentName}, "", nil)
	assert.Nil(t, err)
	assert.Equal(t, 1, count)
	assert.Equal(t, 1, len(clustersWithEnvAndRegion))
	assert.Equal(t, cluster.Name, clustersWithEnvAndRegion[0].Name)
	assert.Equal(t, cluster.EnvironmentName, clustersWithEnvAndRegion[0].EnvironmentName)
	assert.Equal(t, cluster.RegionName, clustersWithEnvAndRegion[0].RegionName)
	...
	count, clustersWithEnvAndRegion, err = Mgr.ListByNameFuzzily(ctx, cluster.EnvironmentName, "clu",
	...
	clusterCountForUser, clustersForUser, err := Mgr.ListUserAuthorizedByNameFuzzily(ctx,
		cluster.EnvironmentName, "clu", []uint{applicationID}, user2.ID, nil)
	assert.Nil(t, err)
	...
	clusterCountForUser, clustersForUser, err = Mgr.ListUserAuthorizedByNameFuzzily(ctx,
		cluster.EnvironmentName, "clu", []uint{applicationID}, user2.ID,
