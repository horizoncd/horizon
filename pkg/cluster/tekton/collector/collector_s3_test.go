...
func TestS3Collector_Collect(t *testing.T) {
	...
	// collect
	err = c.Collect(ctx, pr)
	assert.Nil(t, err)

	// 验证collect
	// 1. getLatestPipelineRunLog
	b, err := c.GetLatestPipelineRunLog(ctx, application, cluster)
	assert.Nil(t, err)
	t.Logf(string(b))
	// 2. getLatestPipelineRunObject
	obj, err := c.GetLatestPipelineRunObject(ctx, application, cluster)
	assert.Nil(t, err)
	assert.NotNil(t, obj)
	...
	// delete
	if err := c.Delete(ctx, application, cluster); err != nil {
		t.Fatal(err)
	}
