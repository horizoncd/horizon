...
func Test(t *testing.T) {
	...
	baseParams := &BaseParams{
		...
		TemplateRelease: &appmodels.TemplateRelease{
		...
		Environment: "test",
		RegionEntity: &appmodels.RegionEntity{
			Region: &appmodels.Region{
				Name:        "hz",
				DisplayName: "HZ",
				Server:      "https://k8s.com",
			},
			Registry: &appmodels.Registry{
	...
	err = r.UpdateTags(ctx, application, cluster, templateName, []*appmodels.Tag{
		{
			Key:   "a",
			Value: "b",
		},
...
func TestUpgradeToV2(t *testing.T) {
	...
	upgradeCommit, err := r.UpgradeCluster(ctx, &UpgradeValuesParam{
		...
		Cluster:     cluster,
		Template:    template,
		TargetRelease: &appmodels.TemplateRelease{
