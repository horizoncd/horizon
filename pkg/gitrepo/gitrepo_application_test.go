...
func createApplicationGitRepoCtx() (context.Context, gitlablib.Interface,
	string, string, *gitlab.Group, string, map[string]interface{}, string, map[string]interface{},
	string, map[string]interface{}, string, map[string]interface{}, string, error) {
	var (
		ctx           context.Context
	...
	if param == "" {
		return nil, nil, "", "", nil, "", nil, "", nil, "", nil, "", nil, "", fmt.Errorf("Env not found")
	}
	var p *ApplicationGitRepoParam
	...
	return ctx, g, defaultBranch, rootGroupName, rootGroup, app,
		pipelineJSONBlob, pipelineJSONStr, applicationJSONBlob, applicationJSONStr,
		pipelineJSONBlob2, pipelineJSONStr2, applicationJSONBlob2, applicationJSONStr2, err
}

type ApplicationGitRepoParam struct {
	Token         string `json:"token"`
	BaseURL       string `json:"baseURL"`
	RootGroupName string `json:"rootGroupName"`
}

func TestApplicationGitRepoV2(t *testing.T) {
	ctx, g, defaultBranch, rootGroupName, rootGroup, app, pipelineJSONBlob, _,
		applicationJSONBlob, _, pipelineJSONBlob2, _, applicationJSONBlob2, _, err := createApplicationGitRepoCtx()
	if err != nil {
		return
	}
