...
type TokenService interface {
	...
	CreateAccessToken(ctx context.Context, name, expiresAtStr string,
		userID uint, scopes []string) (*models.Token, error)
	CreateJWTToken(subject string, expiresIn time.Duration, options ...models.ClaimsOption) (string, error)
	ParseJWTToken(tokenStr string) (models.Claims, error)
}

func NewTokenService(manager *managerparam.Manager, config tokenconfig.Config) TokenService {
	return &tokenService{
...
func (s *tokenService) CreateAccessToken(ctx context.Context, name, expiresAtStr string,
	userID uint, scopes []string) (*models.Token, error) {
	// 1. check expiration date
	createdAt := time.Now()
	expiresIn := time.Duration(0)
	if expiresAtStr != models.NeverExpire {
		expiredAt, err := time.Parse(models.ExpiresAtFormat, expiresAtStr)
		if err != nil {
...
func (s *tokenService) genAccessToken(gen generator.AccessTokenCodeGenerator, name string, userID uint,
	scopes []string, createdAt time.Time, expiresIn time.Duration) (*models.Token, error) {
...
func (s *tokenService) CreateJWTToken(subject string,
	expiresIn time.Duration, options ...models.ClaimsOption) (string, error) {
	now := time.Now().UTC()
	claims := &models.Claims{
		RegisteredClaims: jwt.RegisteredClaims{
			Issuer:    models.ClaimsIssuer,
			Subject:   subject,
...
func (s *tokenService) ParseJWTToken(tokenStr string) (models.Claims, error) {
	...
	if claims.Issuer != models.ClaimsIssuer {
		return models.Claims{}, perror.Wrapf(herror.ErrTokenInvalid,
