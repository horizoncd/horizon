...
func createMemberCtx() (context.Context, *managerparam.Manager, *gorm.DB) {
	var (
		ctx     context.Context
		db      *gorm.DB
	...
	err := db.AutoMigrate(&applicationmodels.Member{},
		&applicationmodels.Group{},
		&applicationmodels.User{},
		&applicationmodels.Application{},
		&applicationmodels.Cluster{},
		&applicationmodels.Template{},
		&applicationmodels.Webhook{},
		&applicationmodels.WebhookLog{},
	)
	if err != nil {
		panic(err)
	}
...
func PostMemberEqualsMember(postMember applicationmodels.PostMember, member *applicationmodels.Member) bool {
	return applicationmodels.ResourceType(postMember.ResourceType) == member.ResourceType &&
...
func TestCreateAndUpdateGroupMember(t *testing.T) {
	ctx, manager, _ := createMemberCtx()

	// mock the groupManager
	mockCtrl := gomock.NewController(t)
	defer mockCtrl.Finish()
	groupManager := groupmanagermock.NewMockGroupManager(mockCtrl)
	roleMockService := rolemock.NewMockService(mockCtrl)
	originService := &memberService{
	...
	// insert memberService to group2
	postMemberTom2 := applicationmodels.PostMember{
	...
	postMemberJerry2 := applicationmodels.PostMember{
		ResourceType: common.ResourceGroup,
		ResourceID:   group2ID,
		MemberInfo:   jerryID,
		MemberType:   applicationmodels.MemberUser,
	...
	postMemberTom1 := applicationmodels.PostMember{
		ResourceType: common.ResourceGroup,
		ResourceID:   group1ID,
		MemberInfo:   tomID,
		MemberType:   applicationmodels.MemberUser,
	...
	postMemberJerry1 := applicationmodels.PostMember{
		ResourceType: common.ResourceGroup,
		ResourceID:   group1ID,
		MemberInfo:   jerryID,
		MemberType:   applicationmodels.MemberUser,
	...
	postMemberCat1 := applicationmodels.PostMember{
		ResourceType: common.ResourceGroup,
		ResourceID:   group1ID,
		MemberInfo:   catID,
		MemberType:   applicationmodels.MemberUser,
	...
	groupManager.EXPECT().GetByID(gomock.Any(),
		gomock.Any()).DoAndReturn(func(_ context.Context, id uint) (*applicationmodels.Group, error) {
		return &applicationmodels.Group{
			Model:           global.Model{},
			Name:            "",
			Path:            "",
			VisibilityLevel: "",
	...
	postMemberCat2 := applicationmodels.PostMember{
		ResourceType: common.ResourceGroup,
		ResourceID:   group2ID,
		MemberInfo:   catID,
		MemberType:   applicationmodels.MemberUser,
...
func TestListGroupMember(t *testing.T) {
	...
	//  case  /group1/group2
	//    group1 memberService: tom(1), jerry(1), cat(1)
	//    group2 memberService: tom(2), jerry(2)
	...
	// insert memberService to group1
	postMemberTom1 := applicationmodels.PostMember{
		ResourceType: common.ResourceGroup,
		ResourceID:   group1ID,
		MemberInfo:   tomID,
...
func TestListApplicationInstanceMember(t *testing.T) {
	ctx, manager, db := createMemberCtx()
	...
	applicationManager := applicationmanagermock.NewMockApplicationManager(mockCtrl)
	...
	clusterManager := clustermanagermock.NewMockClusterManager(mockCtrl)
	clusterManager.EXPECT().GetByID(gomock.Any(),
		gomock.Any()).DoAndReturn(func(_ context.Context, id uint) (*applicationmodels.Cluster, error) {
		return &applicationmodels.Cluster{
			Model:         global.Model{},
			Name:          "",
			Description:   "",
	...
	// insert members
	postMembers := []applicationmodels.PostMember{
		{
			ResourceType: common.ResourceGroup,
			ResourceID:   group1ID,
			MemberInfo:   sphID,
...
func TestGetPipelinerunMember(t *testing.T) {
	...
	pipelineMockManager.EXPECT().GetByID(gomock.Any(), pipelineRunID).Return(&applicationmodels.Pipelinerun{
		ID:        0,
...
func TestListTemplateMember(t *testing.T) {
	...
	group1 := &applicationmodels.Group{
		Name:            "test",
		Path:            "test",
		VisibilityLevel: "",
		Description:     "test",
	...
	group2 := &applicationmodels.Group{
		Name:            "test2",
		Path:            "test2",
		VisibilityLevel: "",
		Description:     "test2",
	...
	template := &applicationmodels.Template{
		Name:       "javaapp",
		Repository: "repo",
		GroupID:    group2.ID,
	}
	...
	_, err = manager.MemberManager.Create(ctx, &applicationmodels.Member{
		ResourceType: applicationmodels.TypeGroup,
...
func TestListWebhookMember(t *testing.T) {
	...
	webhook := &applicationmodels.Webhook{
		URL:          "test",
		ResourceType: "groups",
		ResourceID:   group1.ID,
	}
	...
	webhookLog := &applicationmodels.WebhookLog{
