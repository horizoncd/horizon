...
import (
	...
	webhookmodels "github.com/horizoncd/horizon/pkg/models"
	"gopkg.in/yaml.v3"
...
func (w *worker) setWebhook(webhook *webhookmodels.Webhook) {
	w.webhook.Store(webhook)
}

func (w *worker) getWebhook() (*webhookmodels.Webhook, error) {
	webhook, ok := w.webhook.Load().(*webhookmodels.Webhook)
	if !ok {
...
func NewWebhookServiceService(ctx context.Context, manager *managerparam.Manager,
	config webhookconfig.Config) WebhookService {
...
func (s *webhookService) stopWorkersAndWait() {
	wg := sync.WaitGroup{}
...
func (s *webhookService) reconcileWorkers() {
...
// start webhook webhookService to reconcile workers
func (s *webhookService) Start() {
	go func() {
		defer func() {
			if err := recover(); err != nil {
				log.Errorf(s.ctx, "webhook webhookService reconcile panic: %v", err)
				common.PrintStack()
			}
		}()
...
func (s *webhookService) StopAndWait() {
	s.quit <- true
	<-s.quit
	log.Info(s.ctx, "webhook webhookService stopped")
}

func newWebhookWorker(webhookMgr manager.WebhookManager,
	eventMgr manager.EventManager, userMgr manager.UserManager,
	webhook *webhookmodels.Webhook, idleWaitInterval uint,
...
func (w *worker) sendWebhook(ctx context.Context, wl *webhookmodels.WebhookLog) *webhookmodels.WebhookLog {
	...
	for {
		...
		default:
			...
			wls, err := w.webhookManager.ListWebhookLogsByStatus(ctx, webhook.ID,
				webhookmodels.WebhookStatusWaiting)
			if err != nil {
			...
			for _, wl := range wls {
				saveResult := func() {
					if wl.ErrorMessage != "" {
						wl.Status = webhookmodels.WebhookStatusFailed
					} else {
						wl.Status = webhookmodels.WebhookStatusSuccess
					}
