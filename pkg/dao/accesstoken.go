...
func (d *accessTokenDAO) ListAccessTokensByResource(ctx context.Context, resourceType string, resourceID uint,
	query *q.Query) ([]*models.AccessToken, int, error) {
	var (
	...
	result := d.db.WithContext(ctx).Table("tb_token as t").
		...
		Where("u.user_type = ?", models.UserTypeRobot).
...
func (d *accessTokenDAO) ListPersonalAccessTokens(ctx context.Context,
	query *q.Query) ([]*models.AccessToken, int, error) {
	var (
		pageSize   = common.DefaultPageSize
		pageNumber = common.DefaultPageNumber
